...
python -m code_.preprocessing.copy_files %root% "data/files_ar" -ar

ECHO Copying edf files from "02_tcp_le" to folder "files_le"
python -m code_.preprocessing.copy_files %root% "data/files_le" -le
...
for /f "delims==" %%a IN ('dir /b /s "data/files_ar"') do (
    call python -m code_.preprocessing.nedc_pystream -p params_04.txt -export "data/files_processed_ar" "%%a"
...
for /f "delims==" %%a IN ('dir /b /s "data/files_le"') do (
    call python -m code_.preprocessing.nedc_pystream -p params_05.txt -export "data/files_processed_le" "%%a"
    echo %%a processed successfully
)
ECHO Completed
