...
def preprocess_data(fsamp_mont, sig_mont, labels_mont):

    # resample the signal to 250 Hz if the sampling rate is different
    if fsamp_mont[0] != SAMPLING_RATE:
        # get the seconds
        secs = len(sig_mont[0,:])/fsamp_mont[0]
        # resample
        sig_mont = resample(sig_mont, int(secs * SAMPLING_RATE), axis=1)

    try:
        # make an empty file of 0's
        sigbufs = np.zeros((len(sig_mont[0,:]), len(labels_mont)))

        # for each of the channels in the selected channels
        for i, channel in enumerate(labels_mont):
            try:
                # add the channel data into the array
                sigbufs[:, i] = sig_mont[i, :]
        ...
        df = pd.DataFrame(sigbufs, columns=labels_mont).astype('float32')
...
def main(argv):
    ...
    # preprocess the data
    df, samp_freq = preprocess_data(fsamp_mont, np.asarray(sig_mont), labels_mont)

    # load export directory
    export_dir = known_args.export
    if export_dir == None:
        print("(%s: %s) error loading export directory" \
            % (sys.argv[0], __name__))
        sys.exit(-1)
    if not os.path.exists(export_dir):
        os.makedirs(export_dir)

    # save the dataframe as a pickle file for quick access of the data in another file
    df.to_pickle(export_dir + "/" + known_args.args[0][-22:].replace(".edf", "") + ".pkl")
    # exit gracefully
    #
